...
function quoteFor(amount, unit) {
  ...
  // Same semantics as `quote::quote_for`: dollars are what the payee receives
  // and the fees ride on top, ZEC is what leaves the wallet.
  const typedZat = unit === 'usd'
    ? Math.round((n / RATE_USD_PER_ZEC) * 1e8)
    : Math.round(n * 1e8);
  const probeFee = platformFee(typedZat);
  const miner = E.releaseFee(REDEEM_LEN, probeFee > 0 ? 2 : 1);

  let amountZat = typedZat;
  if (unit === 'usd') {
    // The platform fee is a share of the number being solved for, so this
    // iterates to the fixed point rather than dividing.
    const base = typedZat + miner;
    amountZat = base;
    for (let i = 0; i < 64; i++) {
      const need = base + platformFee(amountZat);
      if (need === amountZat) break;
      amountZat = need;
    }
  }
  ...
  const netCents = unit === 'usd'
    ? Math.round(n * 100)
    : grossCents - feeCents - minerCents;
