...
fn an_announcement_from_an_unpinned_attestor_is_refused() {
    ...
    assert_eq!(
        verify_announcement(&ann, &impostor),
        Err(ClientError::UnpinnedAttestor)
    );
    verify_announcement(&ann, &d.public_key(&secp)).unwrap();
}
...
fn the_refund_is_refused_before_t_and_built_at_t() {
    ...
    assert_eq!(
        refund_when_due(&early, &record, &policy, LOCK_HEIGHT, &p2pkh([0x0b; 20]), fee)
            .unwrap_err(),
    ...
    let refund = refund_when_due(&due, &record, &policy, LOCK_HEIGHT, &p2pkh([0x0b; 20]), fee)
...
fn the_refund_is_rebuilt_from_the_stored_record_alone() {
    ...
    let from_record =
        refund_when_due(&chain, &record, &policy, LOCK_HEIGHT, &p2pkh([0x0b; 20]), fee)
            .unwrap()
            .sighash()
            .unwrap();
...
fn a_malformed_redeem_script_is_refused_rather_than_producing_a_wrong_refund() {
    ...
    assert_eq!(
        refund_when_due(&chain, &record, &policy, LOCK_HEIGHT, &p2pkh([0x0b; 20]), 20_000)
            .unwrap_err(),
