...
//! the "per-user read" `0049f5a` named and the thing the pay page cannot
...
impl VenmoBrowser {
    ...
    ///
    /// `GET /api/user/<handle>` is the per-user read `0049f5a`'s message named
...
impl PaymentStep {
    ...
    fn to_expression(&self) -> String {
        match self {
            ...
            // 0da6ff1's message wrongly claimed otherwise: any *signed-in*
